...
void PrintMCST(int T[][V-1], int A[][E]){
    ...
    for (int i {0}; i<V-1; i++){
        cout << "[" << T[0][i] << "]-----[" << T[1][i] << "]" << " Weight: " << A[2][i] << endl;
        sum += A[2][i]; // Add the weight of the current edge to the sum
    }
    cout << "\nSum of weights in the Minimum Cost Spanning Tree: " << sum << endl; // Print the sum
    cout << endl;
}
